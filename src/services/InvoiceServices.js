...
const CreateInvoiceService=async(req)=>{
    ...
    //01 Calculate Total, Payable, Vat Amount
...
const InvoiceListService=async(req)=>{
   try{
    ...
    let InvoiceList=await InvoiceModel.find({userID:user_id})
...
const InvoiceProductListService=async(req)=>{
    try{
        ...
        let InvoiceProductList=await InvoiceProductModel.aggregate([matchStage,JoinStageProduct,unwindProductStage])

        return{status:"success", data:InvoiceProductList};
    }
    catch(e){
        return{status:"fail", data:e.toString()};
    }
